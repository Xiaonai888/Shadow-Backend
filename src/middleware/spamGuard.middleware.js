...
function resolveGuardCode(result) {
  const status = String(result?.block_status || result?.status || '').toLowerCase()

  if (result?.is_permanent_blocked || status === 'permanent_block') {
    return {
  ...
  if (status === 'seven_day_quarantine') {
    return {
      ...
      retryAfter: Math.max(1, Number(result.retry_after_seconds || 604800)),
    }
  }
...
export function createSpamGuard({
  ...
  return async function spamGuardMiddleware(req, res, next) {
    ...
    try {
      ...
      res.setHeader('Retry-After', String(resolved.retryAfter))
