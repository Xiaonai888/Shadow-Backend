  ...
  try {
    ...
    const [
      ...
      supabase.rpc(
        ...
        {
          p_author_page_id: authorPage.id,
        }
      ),
      ...
      supabase.rpc(
        'get_author_dashboard_recent_comments',
        {
          p_author_page_id: authorPage.id,
          p_limit: 3,
        }
    ...
    if (topStoriesResult.error) throw topStoriesResult.error
    if (latestPostResult.error) throw latestPostResult.error
    ...
    const periodTotals = series.reduce(
      (totals, item) => ({
        page_views:
          totals.page_views + item.page_views,
        story_reads:
          totals.story_reads + item.story_reads,
        interactions:
          totals.interactions + item.interactions,
        new_followers:
          totals.new_followers + item.new_followers,
        comments:
    ...
    const postLikes = safeNumber(summary.post_likes)
    const postComments = safeNumber(summary.post_comments)
    const postEchoes = safeNumber(summary.post_echoes)
    const storyViews = safeNumber(summary.story_views)
    ...
    return res.status(200).json({
      ...
      overview: {
        posts: safeNumber(summary.posts),
        stories: safeNumber(summary.stories),
      ...
      analytics: series,
      top_stories: topStoriesResult.data || [],
      latest_post: latestPostResult.data || null,
      recent_comments: recentCommentsResult.data || [],
      notifications: notificationsResult.data || [],
      unread_updates: safeNumber(unreadResult.count),
    })
  } catch (error) {
    console.error(
      'GET MY AUTHOR DASHBOARD ERROR:',
      error
    )
