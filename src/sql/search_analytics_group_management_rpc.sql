create or replace function public.rename_search_analytics_group(
  p_group_id bigint,
...
declare
  v_name text := left(
    regexp_replace(
      trim(coalesce(p_canonical_term, '')),
      '\\s+',
      ' ',
      'g'
    ),
...
begin
  if p_group_id is null or p_group_id <= 0 then
    raise exception 'Valid group id is required';
  end if;
  ...
  if not found then
    raise exception 'Search group not found';
  end if;

  if v_status = 'merged' then
    raise exception 'Merged group cannot be renamed';
  end if;

  update public.search_term_groups
  set
    canonical_term = v_name,
    updated_at = now()
  where id = p_group_id;
...
create or replace function public.set_search_analytics_group_ignored(
  p_group_id bigint,
...
declare
  v_status text;
  v_new_status text := case
    when coalesce(p_ignored, true) then 'ignored'
    else 'active'
  end;
begin
  if p_group_id is null or p_group_id <= 0 then
  ...
  if v_status = 'merged' then
    raise exception 'Merged group cannot be ignored or restored';
  end if;
...
revoke all
on function public.rename_search_analytics_group(bigint, text)
from public, anon, authenticated;

revoke all
on function public.set_search_analytics_group_ignored(bigint, boolean)
from public, anon, authenticated;
