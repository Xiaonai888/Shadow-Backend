...
function publicUser(user) {
  if (!user) return null
  return { id: user.id, name: user.name || '', username: user.username || '', email: user.email || '', avatar_url: user.avatar_url || '' }
}
...
async function getUsersMap(userIds) {
  ...
  const { data, error } = await supabase.from('users').select('id, name, username, email, avatar_url').in('id', ids)
...
export async function getAdminManualPayments(req, res) {
  try {
    ...
    let query = supabase.from('payment_transactions').select('*').eq('payment_method', 'aba_payment_link').order('created_at', { ascending: false }).limit(limit)
...
export async function confirmAdminManualPayment(req, res) {
  try {
    ...
    const { data, error } = await supabase.rpc('confirm_manual_payment', { p_payment_id: paymentId, p_admin_id: getAdminId(req), p_admin_note: adminNote || null })
    if (error) throw error
...
export async function rejectAdminManualPayment(req, res) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .update({ status: 'rejected', admin_reviewed_by: getAdminId(req), admin_reviewed_at: new Date().toISOString(), admin_note: adminNote || null, rejected_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      .eq('id', paymentId)
