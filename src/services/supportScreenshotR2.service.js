...
function safeFileName(value) {
  const original = String(value || 'screenshot').trim()
  const withoutPath = original.split(/[\\/]/).pop() || 'screenshot'
  ...
  return (
    withoutPath
      ...
      .replace(/-+/g, '-')
      .replace(/^-|-$/g, '')
      .slice(-120) || 'screenshot'
  )
}

function createStorageKey(userId, requestId, originalName) {
  const readerId = String(userId || '').trim()
  const ticketId = String(requestId || '').trim()

  if (!readerId || !ticketId) {
    throw new Error('User ID and request ID are required')
  }

  const random = Math.random().toString(36).slice(2, 12)

  return `support-screenshots/${readerId}/${ticketId}/${Date.now()}-${random}-${safeFileName(
    originalName
  )}`
}
...
export async function getSupportScreenshotFromR2(storageKey) {
...
export async function deleteSupportScreenshotFromR2(storageKey) {
  const key = String(storageKey || '').trim()
