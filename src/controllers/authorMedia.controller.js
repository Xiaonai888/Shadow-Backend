...
async function getMyAuthorPage(
  userId
) {
  const { data, error } =
    await supabase
...
async function uploadAuthorImageToR2({
  authorPage,
  file,
  imageType,
}) {
  if (!file?.path) {
    ...
    error.code =
      'AUTHOR_PROFILE_IMAGE_FILE_MISSING'
    throw error
  }
  ...
  const folder =
    imageType === 'cover'
      ...
      : 'avatars'
  const fileName =
    `${imageType}-${Date.now()}-${Math.random()
      .toString(36)
      .slice(2)}.${ext}`
  const filePath =
    `authors/${authorPage.id}/${folder}/${fileName}`
  const publicUrl =
...
export async function getMyAuthorStorageQuota(
  req,
  res
) {
  try {
    ...
    return res.status(200).json({
      ok: true,
      author_page:
        publicAuthorPage(authorPage),
      storage: quota,
    })
  } catch (error) {
    console.error(
      'GET AUTHOR STORAGE QUOTA ERROR:',
      error
    )
    ...
    return res.status(500).json({
      ok: false,
      message:
        'Failed to load author storage quota',
      error: error.message,
    })
  }
}

export async function uploadMyAuthorProfileImage(
  req,
  res
) {
  try {
    ...
    if (!req.file?.path) {
      return res.status(400).json({
        ok: false,
        message:
          'Image file is required. Use form field name: image',
      })
    }
    ...
    if (
      ...
      return res.status(400).json({
        ok: false,
        message:
          'image_type must be avatar or cover',
      })
    }
    ...
    return res.status(200).json({
      ...
      message:
        imageType === 'avatar'
          ? 'Author profile photo uploaded'
          : 'Author cover image uploaded',
      image_type: imageType,
      image_url:
        uploaded.publicUrl,
      author_page:
        publicAuthorPage(updatedPage),
      asset,
      storage: quota,
    })
  } catch (error) {
    console.error(
      'UPLOAD AUTHOR PROFILE IMAGE ERROR:',
      error
    )
    ...
    return res
      ...
      .json({
        ...
        code:
          error.code ||
          'AUTHOR_PROFILE_IMAGE_UPLOAD_FAILED',
        message:
          error.message ||
          'Failed to upload author profile image',
        quota:
          error.quota || null,
        requested_bytes:
