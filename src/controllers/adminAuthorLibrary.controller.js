...
async function serveAdminAuthorLibraryPdf(req, res, mode) {
  setPrivateHeaders(res)
  const productId = String(req.params.productId || '')
  const action = mode === 'read' ? 'read_pdf' : 'download_pdf'
  if (!UUID.test(productId)) return res.status(400).json({ ok: false, message: 'Invalid PDF ID' })
  try {
    if (mode === 'download') {
      const pin = String(req.body?.pin || '')
      if (!/^\d{6}$/.test(pin)) {
        await audit(req, productId, 'blocked', 'Admin Passkey PIN required', action)
        return res.status(400).json({ ok: false, message: 'Enter your 6-digit Admin Passkey PIN' })
      }
      const checked = await verifyAdminPasskeyPin({ admin: req.admin, req, pin, purpose: `author_library_pdf_download:${productId}` })
    ...
    if (!product || product.product_type !== 'pdf' || product.pdf_is_private !== true) {
      await audit(req, productId, 'blocked', 'Private PDF unavailable', action)
    ...
    if (!UUID.test(String(product.author_page_id || '')) || !key.startsWith(`author-private-pdfs/${product.author_page_id}/`)) {
      await audit(req, productId, 'blocked', 'Invalid private PDF storage path', action)
    ...
    if (first.done || Buffer.from(first.value).subarray(0, 5).toString('latin1') !== '%PDF-') {
      source.destroy?.()
      await audit(req, productId, 'blocked', 'PDF signature invalid', action)
      return res.status(422).json({ ok: false, message: 'Uploaded file is not a readable PDF; author must replace it' })
    }
    await audit(req, productId, 'success', `Authenticated private PDF ${mode} stream started`, action)
    const name = String(product.pdf_file_name || `${product.title || 'document'}.pdf`).split(/[\\/]/).pop().replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 100)
    res.set('Content-Type', 'application/pdf')
    res.set('Content-Disposition', `${mode === 'read' ? 'inline' : 'attachment'}; filename="${(name.toLowerCase().endsWith('.pdf') ? name : `${name}.pdf`).replace(/"/g, '')}"`)
...
export async function downloadAdminAuthorLibraryPdf(req, res) {
  return serveAdminAuthorLibraryPdf(req, res, 'download')
}

export async function readAdminAuthorLibraryPdf(req, res) {
  return serveAdminAuthorLibraryPdf(req, res, 'read')
}
