...
import {
  getDiscoverAuthorPostsSharedCatalog,
...
function getDecayedInterestScore(interest, snapshotAt) {
  const score = Math.max(
    0,
    Number(interest?.interest_score || 0)
  )
  ...
  const ageDays =
    Math.max(0, snapshotTime - signalTime) /
    (24 * 60 * 60 * 1000)
  ...
  return (
    ...
    Math.pow(
      0.5,
      ageDays / INTEREST_HALF_LIFE_DAYS
    )
  )
}
...
function buildPersonalizationByPost(
  ...
  const interestByHashtag = new Map()

  for (const item of userInterests || []) {
    const hashtagId = String(item?.hashtag_id || '')
    if (!hashtagId) continue

    const score = getDecayedInterestScore(item, snapshotAt)
    if (score <= 0.01) continue

    interestByHashtag.set(hashtagId, score)
  }

  if (!interestByHashtag.size) return new Map()

  const matchedScoresByPost = new Map()

  for (const row of postHashtags || []) {
    const postId = String(row?.post_id || '')
    const hashtagId = String(row?.hashtag_id || '')
    const score = interestByHashtag.get(hashtagId)

    if (!postId || !hashtagId || !score) continue

    if (!matchedScoresByPost.has(postId)) {
      matchedScoresByPost.set(postId, [])
    }

    matchedScoresByPost.get(postId).push(score)
  }

  const personalizationByPost = new Map()

  for (const [postId, scores] of matchedScoresByPost.entries()) {
    const strongestMatches = [...scores]
      .sort((first, second) => second - first)
      .slice(0, MAX_MATCHED_INTERESTS)

    const matchedInterest = Math.min(
      100,
      strongestMatches.reduce((sum, score) => sum + score, 0)
    )

    const boost =
      matchedInterest > 0
        ? (Math.log1p(matchedInterest) / Math.log1p(100)) *
          MAX_PERSONALIZATION_BOOST
        : 0

    if (boost > 0) {
      personalizationByPost.set(postId, boost)
    }
  }
...
function getRecommendationScore(
  ...
  const snapshotTime = new Date(snapshotAt).getTime()
  const createdTime = new Date(post?.created_at || 0).getTime()
  const ageMs =
    Number.isFinite(snapshotTime) && Number.isFinite(createdTime)
      ? Math.max(0, snapshotTime - createdTime)
      : 0
  const ageDays = ageMs / (24 * 60 * 60 * 1000)
  const likes = Math.max(0, Number(post?.like_count || 0))
  const comments = Math.max(0, Number(post?.comment_count || 0))
  const echoes = Math.max(0, Number(post?.echo_count || 0))
  const engagement = likes + comments * 2 + echoes * 3
  const engagementScore = Math.log1p(engagement) * 12
  const recencyScore = Math.max(0, 18 - ageDays * 0.6)
  const discoveryBoost = authorPage?.is_following ? 0 : 2
  const ownerBoost = authorPage?.is_owner ? 1 : 0
  const personalizationScore = authorPage?.is_owner
    ? 0
    : Math.min(
        MAX_PERSONALIZATION_BOOST,
        Math.max(0, Number(personalizationBoost || 0))
      )
...
function compareRecommended(
  ...
  const firstAuthor = authorById.get(String(first.author_page_id))
  const secondAuthor = authorById.get(String(second.author_page_id))

  const firstScore = getRecommendationScore(
    first,
    firstAuthor,
    snapshotAt,
    personalizationByPost.get(String(first.id)) || 0
  )
  const secondScore = getRecommendationScore(
    second,
    secondAuthor,
    snapshotAt,
    personalizationByPost.get(String(second.id)) || 0
  )
...
function buildReactionData(rows, userId) {
  ...
  for (const row of rows || []) {
    ...
    counts.set(
      reactionType,
      Number(counts.get(reactionType) || 0) + 1
    )
...
export async function getDiscoverAuthorPostsFeed(req, res) {
  try {
    ...
    const catalog = await getDiscoverAuthorPostsSharedCatalog({
      snapshotAt: cursor?.snapshot_at || '',
    })

    const snapshotAt = catalog.snapshotAt
    ...
    const [
      ...
      supabase
        ...
        .select('hashtag_id, interest_score, last_signal_at')
        .eq('user_id', userId)
        .gt('interest_score', 0)
        .order('interest_score', { ascending: false })
        .limit(USER_INTEREST_LIMIT),
    ])
    ...
    const personalizationByPost = buildPersonalizationByPost(
    ...
    if (pageUserIds.length) {
      const { data, error } = await supabase
        ...
        .in('blocked_user_id', pageUserIds)
    ...
    const blockedPageIds = new Set(
      (pageBlocksResult.data || []).map((item) =>
        String(item.author_page_id)
      )
    )
    const blockedUserIds = new Set(
      accountBlocks.map((item) => String(item.blocked_user_id))
    )
    ...
    const authorById = new Map(
      pages
        .filter(
          (page) =>
            !blockedPageIds.has(String(page.id)) &&
        ...
        .map((page) => [
          String(page.id),
          {
            ...
            is_following: followedPageIds.has(String(page.id)),
            is_owner: String(page.user_id) === String(userId),
          },
        ])
    )
    ...
    const selectedPosts = visiblePosts.slice(
      offset,
      offset + limit
    )
    ...
    if (selectedPostIds.length) {
      const [reactionResult, echoResult] = await Promise.all([
        supabase
      ...
      for (const row of echoResult.data || []) {
        ...
        echoCountByPost.set(
          postId,
          Number(echoCountByPost.get(postId) || 0) +
    ...
    const posts = selectedPosts.map((post) => {
      ...
      const authorPage =
        authorById.get(String(post.author_page_id)) || null
      ...
      return {
        ...
        echo_count: Number(echoCountByPost.get(postId) || 0),
        echo_state_loaded: true,
        reaction_summary: summaryByPost.get(postId) || [],
        my_reaction: myReactionByPost.get(postId) || null,
        created_at: post.created_at,
        updated_at: post.updated_at,
        is_following: Boolean(authorPage?.is_following),
        is_owner: Boolean(authorPage?.is_owner),
        author_page: authorPage,
      }
    })

    const nextOffset = offset + posts.length
    const hasMore = nextOffset < visiblePosts.length
    ...
    return res.status(500).json({
      ok: false,
      message: 'Failed to load discover author posts',
      error: error.message,
    })
  }
}
